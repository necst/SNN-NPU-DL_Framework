...
def snn_neuron(dev, in1_size, out_size, threshold, decay_factor, reset, hard_reset, trace_size):
    ...
    hidden_1_output_ty = np.ndarray[(tile_size,), np.dtype[np.int32]]
    output_layer_ty = np.ndarray[(tile_size,), np.dtype[np.int32]]
    ...
    def core_body(of_in_spikes, of_out_spikes, of_in_membrane, of_out_membrane, of_in_weights_L2_L1, input_layer_size, output_layer_size, lif_neuron):
        ...
        for _ in range_(number_sub_vectors):
            ...
            lif_neuron(elem_in_spikes, elem_out, elem_in_membrane, elem_out_membrane, weights, input_layer_size, output_layer_size, threshold, decay_factor, reset, hard_reset, input_layer)
    ...
    snn_kernel_input_hidden = Kernel("snnNeuronLineSimdInputHidden", "scale.o", [input_tile_ty, hidden_1_output_ty, membrane_ty, membrane_ty, weight_input_hidden_ty, np.int32, np.int32, np.float32, np.float32, np.float32, np.int32, np.int32],)
    snn_kernel_hidden_output = Kernel("snnNeuronLineSimdHiddenOutput", "scale.o", [hidden_1_output_ty, output_layer_ty, membrane_ty, membrane_ty, weight_hidden_output_ty, np.int32, np.int32, np.float32, np.float32, np.float32, np.int32, np.int32],)


    # Input -> Hidden_1
    workers.append(Worker(core_body, fn_args=[of_in_spikes_L2_L1.cons(), of_out_spikes_L1_L2[0].prod(), of_in_membrane[0].cons(), of_in_membrane[0].prod(), of_in_weight_L2_L1[0].cons(), input_layer, hidden_layer_1, snn_kernel_input_hidden]))

    # Hidden_1 -> Output
    workers.append(Worker(core_body, fn_args=[of_out_spikes_L1_L2[0].cons(), of_out_spikes_L1_L2[1].prod(), of_in_membrane[1].cons(), of_in_membrane[1].prod(), of_in_weight_L2_L1[1].cons(), hidden_layer_1, output_layer, snn_kernel_hidden_output]))
