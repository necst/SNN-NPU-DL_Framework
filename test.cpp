...
int main(int argc, const char *argv[]) {
  ...
  for (int i = 0; i < IN_SIZE; ++i) { // Fixed loop variable
    buf_in_spikes[i] = dist(gen) ? 1 : 0;
  }

  // Initialize threshold in input
  DATATYPE *buf_threshold = bo_threshold.map<DATATYPE *>();
  buf_threshold[0] = threshold; // Fixed assignment

  // Zero out buffer for the output spikes
  DATATYPE *buf_out_spikes = bo_out_spikes.map<DATATYPE *>();
  memset(buf_out_spikes, 0, OUT_SIZE * sizeof(DATATYPE)); // Fixed memset
