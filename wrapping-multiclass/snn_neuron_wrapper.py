...
class base_neuron:
    ...
    def to(self, device = "npu"):
        self.device = device
        if(device == "npu"):
            compile_snn_neuron(in1_size=self.in1_size, out_size=self.out_size, threshold=self.threshold, decay_factor= self.decay_factor, reset=self.reset, hard_reset=self.hard_reset, trace_size=self.trace_size, device=self.device, aie_degign_name = "snn_neuron", use_placed=self.use_placed)
            self.compiled = True
...
class snn_neuron_npu_multicore(base_neuron):
    # Overload
    def __init__(self, in1_size=4096, out_size=4096, threshold=10, decay_factor=0.9, reset=-1, hard_reset=1, trace_size=8192, use_placed=False, aie_design_name = "multicore", num_cores = 2):
...
class snn_neuron_npu_singlecore(base_neuron):
    # Overload
    def __init__(self, in1_size=4096, out_size=4096, threshold=10, decay_factor=0.9, reset=-1, hard_reset=1, trace_size=8192, use_placed=False, aie_design_name = "singlecore"):
        super().__init__(in1_size=in1_size, out_size=out_size, threshold=threshold, decay_factor=decay_factor, reset=reset, hard_reset=hard_reset, trace_size=trace_size, use_placed=use_placed)
