...
def my_vector_bias_add():
    ...
    of_in_threshold = ObjectFifo(threshold_ty, name="in_threshold", depth = 1)
    ...
    with rt.sequence(all_data_ty, threshold_ty, all_data_ty) as (inTensor, inThreshold, outTensor):
        rt.start(worker)
        rt.fill(of_input_spikes.prod(), inTensor)
        rt.fill(of_input_threshold, inThreshold)
        rt.drain(of_output_spiks.cons(), outTensor, wait=True)
